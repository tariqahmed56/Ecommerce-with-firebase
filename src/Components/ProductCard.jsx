...
const ProductCard = ({
  ...
  return (
    <div className="flex flex-col w-[200px] relative h-[340px] cursor-pointer text-black border border-gray-200 rounded-md">
      {stock === 0 && (
        <div className="sold-out-tag absolute z-10 px-2  w-[100px] py-3 -translate-x-2 translate-y-[10px] rotate-[30deg] rounded-md text-white font-semibold bg-red-500 top-1 left-2">
          sold Out
        </div>
      )}
      <AiOutlineHeart
      ...
      <p className="text-[12px] font-normal whitespace-nowrap text-nowrap overflow-hidden text-ellipsis px-2">
        {title}
      </p>
      {isShippingFree && (
        <p className="text-[10px] font-medium text-gray-500 text-ellipsis uppercase px-2">
          Delivery free
        </p>
      )}
      <div className="price flex gap-1 items-center px-2">
        <span>Rs.</span>
        <p className="originalPrice text-[13px] font-semibold text-red-600">
          {actualPrice}
        </p>
        <p className="originalPrice text-[13px] line-through">
          {originalPrice}
        </p>
      </div>
    </div>
